...
The implementations \lstinline{algorithm1} and \lstinline{algorithm2} aren't distinguished by the Numba compiler, and both pay a cost to call subroutines defined outside of their function body. The \textit{nested} function in \lstinline{algorithm3} differs from the other two implementations, by defining its sub-routines within its function body, rather than calling externally defined functions. This is an example of an inlining optimization, which LLVM is capable of picking up at compile time. Inlining optimizations are a beneficial technique as they reduce function call overhead and encourage code locality, ultimately enhancing execution speed and overall program performance. However, here Numba optimizes out the matrix-vector product as it's not being stored in a result that is later used in the program. This is reflected in the runtimes of all three implementations, shown in table (\ref{tab:boxing_inlining}) for different problem sizes \footnote{All experiments in this work were taken on an AMD Ryzen Threadripper 3970X 32-Core processor running Python 3.8.5 and Numba 0.53.0}. This example is designed to illustrate how arbitrary changes to writing style can impact the behavior of Numba code. 
...
Therefore, though Numba is advertised as an easy way of injecting performance into your program via simple decorators, it has its own learning curve. Achieving performance requires a programmer to be familiar with the internals of its implementation and potential discrepancies that arise when translating between Python and the LLVM-generated code, which may lead to significant alterations in the design of algorithms and data structures. A performant Numba code in which the number of interactions between Numba and Python is minimized, and inlined code is maximized, could look decidedly un-Pythonic. Additionally, unit testing such code is difficult to its lack of modularity. Numba can be seen to encourage few user-created objects, performance-critical sections written in terms of loops over simple array-based data structures, and potentially long nested functions in order to achieve performance.
